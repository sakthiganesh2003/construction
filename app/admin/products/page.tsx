...
export default function ProductsAdminPage() {
  ...
  return (
    <div className="space-y-5">
      ...
      {saved && <div className="flex items-center gap-2 px-4 py-3 bg-green-500/15 border border-green-500/25 rounded-xl text-green-400 text-sm"><Check size={16} /> Product saved!</div>}

      {/* Card grid — works great on all sizes */}
      {filtered.length === 0 ? (
        <div className="text-center text-slate-500 py-12 text-sm">No products found.</div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
          {filtered.map((p, i) => (
            <div key={p.id} className="bg-white/3 border border-white/8 rounded-2xl p-4 hover:border-white/15 transition-all group">
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-orange-400 font-mono text-xs mt-0.5">{p.model}</p>
                  </div>
                </div>
                <div className="flex gap-1.5 shrink-0 ml-2">
                  <button onClick={() => openEdit(p)} className="p-2 rounded-lg bg-blue-500/15 text-blue-400 hover:bg-blue-500/25 cursor-pointer"><Pencil size={13}/></button>
                  <button onClick={() => setDeleteId(p.id)} className="p-2 rounded-lg bg-red-500/15 text-red-400 hover:bg-red-500/25 cursor-pointer"><Trash2 size={13}/></button>
                </div>
              </div>
              <p className="text-slate-400 text-xs leading-relaxed line-clamp-2 mb-3">{p.shortDescription || 'No description.'}</p>
              <div className="flex gap-1.5 flex-wrap">
                {p.isBestseller && <span className="px-2 py-0.5 bg-amber-500/15 border border-amber-500/25 text-amber-400 text-[10px] font-bold rounded-full">Bestseller</span>}
                {p.isNew && <span className="px-2 py-0.5 bg-green-500/15 border border-green-500/25 text-green-400 text-[10px] font-bold rounded-full">New</span>}
            ...
            <div className="flex-1 px-5 py-5 space-y-4 overflow-y-auto">
              ...
              <div className="flex gap-4">
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  <span className="text-slate-300 text-sm">New Arrival</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer">
              ...
              <div>
                ...
                <input value={form.imageIcon} onChange={e => setForm(f => ({ ...f, imageIcon: e.target.value }))}
                  className="w-20 bg-white/5 border border-white/10 text-white text-xl text-center rounded-xl px-3 py-2.5 focus:outline-none" placeholder="🚛" />
              </div>
            </div>
      ...
      {deleteId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-[#0c1a2e] border border-white/10 rounded-2xl p-6 w-full max-w-sm text-center">
            ...
            <h3 className="text-white font-bold mb-1">Delete Product?</h3>
