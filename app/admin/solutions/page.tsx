...
export default function SolutionsAdminPage() {
  ...
  return (
    <div className="space-y-5">
      ...
      {saved && <div className="flex items-center gap-2 px-4 py-3 bg-emerald-50 border border-emerald-200 rounded-xl text-emerald-800 text-sm font-semibold"><Check size={16} /> Solution saved!</div>}

      {/* Data Table */}
      {filtered.length === 0 ? (
        <div className="text-center text-slate-500 py-12 text-sm bg-white rounded-2xl border border-slate-200">No solutions found.</div>
      ) : (
        <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-xs">
          <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
            <h2 className="text-slate-900 font-bold text-sm">Solutions ({filtered.length})</h2>
            ...
            <table className="w-full text-left min-w-[760px]">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50/80 text-slate-500 text-xs font-semibold">
                  <th className="px-5 py-3.5">Solution & Code</th>
                  ...
                  <tr key={s.id} className="hover:bg-slate-50/70 transition-colors">
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-slate-900 font-bold text-sm leading-tight">{s.name}</p>
                          <p className="text-orange-600 font-mono text-xs font-semibold mt-0.5">{s.model}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-5 py-4 text-slate-600 text-xs font-medium max-w-xs truncate">{s.shortDescription || '—'}</td>
                    <td className="px-5 py-4">
                      <div className="flex gap-1.5 flex-wrap">
                        {s.isBestseller && <span className="px-2.5 py-0.5 bg-amber-100 border border-amber-200 text-amber-800 text-[10px] font-bold rounded-full">Bestseller</span>}
                        {s.isNew && <span className="px-2.5 py-0.5 bg-emerald-100 border border-emerald-200 text-emerald-800 text-[10px] font-bold rounded-full">New</span>}
                        {!s.isBestseller && !s.isNew && <span className="px-2.5 py-0.5 bg-slate-100 border border-slate-200 text-slate-500 text-[10px] font-semibold rounded-full">Standard</span>}
                      </div>
                    </td>
                    <td className="px-5 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button onClick={() => openEdit(s)} className="p-1.5 rounded-lg bg-sky-50 text-sky-600 border border-sky-100 hover:bg-sky-100 cursor-pointer" title="Edit"><Pencil size={14}/></button>
                        <button onClick={() => setDeleteId(s.id)} className="p-1.5 rounded-lg bg-red-50 text-red-600 border border-red-100 hover:bg-red-100 cursor-pointer" title="Delete"><Trash2 size={14}/></button>
                      </div>
                    </td>
                  </tr>
                ))}
        ...
        <div className="fixed inset-0 z-50 flex">
          ...
          <div className="w-full max-w-sm bg-white border-l border-slate-200 h-full flex flex-col shadow-2xl">
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
              <h2 className="text-slate-900 font-bold text-base">{editId ? 'Edit Solution' : 'Add Solution'}</h2>
            ...
            <div className="flex-1 px-5 py-5 space-y-4 overflow-y-auto">
              ...
              <div className="flex gap-4">
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  <span className="text-slate-700 text-sm font-medium">New</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer">
              ...
              <div>
                ...
                <input value={form.imageIcon} onChange={e => setForm(f => ({ ...f, imageIcon: e.target.value }))}
                  className="w-20 bg-slate-50 border border-slate-200 text-slate-900 text-xl text-center rounded-xl px-3 py-2.5 focus:outline-none focus:border-orange-500" />
              </div>
            </div>
      ...
      {deleteId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white border border-slate-200 rounded-2xl p-6 w-full max-w-sm text-center shadow-2xl">
            ...
            <h3 className="text-slate-900 font-bold text-base mb-1">Delete Solution?</h3>
