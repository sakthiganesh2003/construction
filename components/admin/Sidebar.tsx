...
export default function AdminSidebar({
  ...
  return (
    <aside
      className={`fixed top-0 left-0 h-screen z-40 flex flex-col w-60 bg-[#1F2937] border-r border-slate-700/60 transition-transform duration-300 ease-in-out
        ${mobileOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}
    >
      {/* Brand Header */}
      <div className="flex items-center gap-3 px-4 py-4 border-b border-slate-700/60 shrink-0">
        ...
        <div className="flex-1 overflow-hidden">
          ...
          <p className="text-orange-400 text-[10px] uppercase tracking-widest font-semibold">Admin Panel</p>
        </div>
      ...
      <nav className="flex-1 overflow-y-auto py-3 scrollbar-thin">
        {navGroups.map(group => (
          <div key={group.label} className="mb-3">
            <p className="text-slate-400 text-[10px] font-bold uppercase tracking-widest px-5 py-1.5">{group.label}</p>
                ...
                return (
                  <li key={href}>
                    <Link
                      href={href}
                      onClick={() => setMobileOpen(false)}
                      className={`flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-sm font-medium transition-all duration-150 relative group
                        ${isActive
                          ? 'bg-[#0EA5E9] text-white font-semibold shadow-md shadow-sky-500/25'
                          : 'text-[#E5E7EB] hover:text-white hover:bg-slate-700/50'
                        }`}
                    >
                      <Icon size={17} className={`shrink-0 ${isActive ? 'text-white' : 'text-slate-300 group-hover:text-white'}`} />
                      <span className="truncate">{label}</span>
                      {isActive && (
                        <ChevronRight size={14} className="ml-auto text-white shrink-0" />
                      )}
                    </Link>
                  </li>
                );
      ...
      <div className="px-4 py-3 border-t border-slate-700/60 shrink-0">
        <p className="text-slate-400 text-[11px]">Veera Blue Metals v1.0 © 2026</p>
      </div>
    </aside>
  );
}
